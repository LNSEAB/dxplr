...
fn main() {
    ...
    let pipeline = {
        let file = File::open("examples/d3d12/compute/compute.hlsl").unwrap();
    ...
    unsafe {
        ...
        std::ptr::copy_nonoverlapping(
            ...
            (std::mem::size_of::<u32>() * 10) as usize,
        );
        upload_buffer.unmap(0, None);
    }
    let readback_buffer = device
        .create_committed_resource::<d3d12::Resource>(
            &d3d12::HeapProperties::new().heap_type(d3d12::HeapType::ReadBack),
            None,
            &d3d12::ResourceDesc::new()
                .dimension(d3d12::ResourceDimension::Buffer)
                .width(256)
    ...
    cmd_list.set_compute_root_descriptor_table(
        0,
        descriptor_heap.get_gpu_descriptor_handle_for_heap_start(),
    );
    ...
    unsafe {
        let data = readback_buffer
            .map(0, Some(d3d12::Range::new(0, 256)))
            .unwrap();
        std::ptr::copy_nonoverlapping(
            data.as_ptr(),
