...
impl Renderer {
    fn new(device: &d3d12::Device, wnd: &winit::Window) -> Self {
        ...
        let pipeline = {
            let file = File::open("examples/texture/tex.hlsl").unwrap();
    ...
    fn upload_image(&self, wnd: &winit::Window, image: image::RgbaImage) -> Mesh {
        let vertex_buffer = self.device
            ...
            .unwrap();
        let wnd_size = wnd.get_inner_size().unwrap().to_physical(wnd.get_hidpi_factor());
        unsafe {
        ...
        let resource_heap = self.device.create_descriptor_heap::<d3d12::DescriptorHeap>(
            &d3d12::DescriptorHeapDesc::new()
        ...
        let texture = self.device.create_committed_resource::<d3d12::Resource>(
        ...
        let intermediate = self.device.create_committed_resource::<d3d12::Resource>(
            &d3d12::HeapProperties::new().heap_type(d3d12::HeapType::Upload),
            None,
            &d3d12::ResourceDesc::new()
                .dimension(d3d12::ResourceDimension::Buffer)
        ...
        self.cmd_list.resource_barrier(&[d3d12::ResourceBarrier::Transition {
            flags: None,
            resource: &texture,
        ...
        self.cmd_list.copy_texture_region(&dest, 0, 0, 0, &src, None);
        self.cmd_list.resource_barrier(&[d3d12::ResourceBarrier::Transition {
            flags: None,
            resource: &texture,
            subresource: d3d12::RESOURCE_BARRIER_ALL_SUBRESOURCES,
        ...
        self.cmd_list.close();
        self.cmd_queue.execute_command_lists(&[self.cmd_list.as_command_list()]);
        self.wait();
        Mesh {
    ...
    fn render(&self, mesh: &Mesh) {
        ...
        let rtv_handle = {
            ...
            let offset = self.device.get_descriptor_handle_increment_size(
                d3d12::DescriptorHeapType::RTV
            );
            handle + index * offset as usize
        };
        let swap_chain_desc = self.swap_chain.get_desc1().unwrap();
        self.cmd_allocator.reset().unwrap();
        self.cmd_list.reset(&self.cmd_allocator, Some(&self.pipeline)).unwrap();
        ...
        self.cmd_list.om_set_render_targets(&[rtv_handle], false, None);
        self.cmd_list.clear_render_target_view(rtv_handle, (0.0, 0.0, 0.3, 0.0).into(), None);
        self.cmd_list.set_graphics_root_signature(&self.root_signature);
        self.cmd_list.set_graphics_root_descriptor_table(0, mesh.resource_heap.get_gpu_descriptor_handle_for_heap_start());
        self.cmd_list.ia_set_primitive_topology(d3d::PrimitiveTopology::TriangleList);
...
fn main() {
    ...
    let mesh = renderer.upload_image(&wnd, image::open("examples/texture/sample.png").unwrap().to_rgba());
